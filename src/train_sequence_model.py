...
from src.expression_preprocess import load_expression_train_data
from src.sequence_preprocess import load_sequence_test_data
import  numpy as np


def train_sequence_model(ontology_name,iea_name):
    ...
    # sequence data 和 expression data 的训练集都是Yeast 数据集
    p1_train, p2_train, label_train = load_expression_train_data(ontology_name=ontology_name,iea_name=iea_name)
    p1_test, p2_test, label_test_LRBS,label_test_RRBS = load_sequence_test_data(ontology_name=ontology_name,iea_name=iea_name)
    ...
    def model_test(rnn, loss_func,epoch):
        ...
        pcc_sim_LRBS = np.corrcoef(label_test_LRBS, prediction_similarity)[0][1]  # 与两个蛋白质向量的余弦相似度的相关系数
        pcc_score_LRBS = np.corrcoef(label_test_LRBS, prediction_scores)[0][1]   # 与预测为1的概率的相关系数
        Test_Result_Log_File.write('Epoch: ' + str(epoch) + '\n')
        Test_Result_Log_File.write('PCC_SIM_LRBS: ')
        Test_Result_Log_File.write(str(pcc_sim_LRBS) + '\n')
        Test_Result_Log_File.write('PCC_SCORE_LRBS: ')
        Test_Result_Log_File.write(str(pcc_score_LRBS) + '\n')

        pcc_sim_RRBS = np.corrcoef(label_test_RRBS, prediction_similarity)[0][1]  # 与两个蛋白质向量的余弦相似度的相关系数
        pcc_score_RRBS = np.corrcoef(label_test_RRBS, prediction_scores)[0][1]  # 与预测为1的概率的相关系数
        Test_Result_Log_File.write('Epoch: ' + str(epoch) + '\n')
        Test_Result_Log_File.write('PCC_SIM_RRBS: ')
        Test_Result_Log_File.write(str(pcc_sim_RRBS) + '\n')
        Test_Result_Log_File.write('PCC_SCORE_RRBS: ')
        Test_Result_Log_File.write(str(pcc_score_RRBS) + '\n')

        Test_Result_Log_File.flush()
        f = open(pred_test_score_file+"."+str(epoch), 'w')
        f.write("prediction socre, prediction similarity between two vectors, LRBS, RRBS")
        for i in range(len(prediction_scores)):
            f.write(str(prediction_scores[i]) + ' ' + str(prediction_similarity[i]) + ' ' + str(label_test_LRBS[i]) + ' ' + str(label_test_RRBS[i]) + '\n')
        f.close()
